...
def change_emails_swap(master, skey, sid, statusVar, inFile):
    dic = getDic(inFile, statusVar)
    print(dic)
    if dic == None:
        statusVar.set('You did not select a csv')
...
class Buttons(tk.Frame):
    def __init__(self, master, variables):
        ...
        #################### just testing ########################
        newEmailsButton = tk.Button(master, text='Change Emails (token)', command=
                                    lambda: change_emails_swap(master,variables.keyVar.get(),variables.surveyListVar.get(),variables.statusVar, variables.csvVar.get()))
        newEmailsButton.grid(row=3, column=2, sticky=tk.W+tk.E)
        removeEmailsButton = tk.Button(master, text='Change Emails (email)', command=
                                       lambda: change_emails(master,variables.keyVar.get(),variables.surveyListVar.get(),variables.statusVar, variables.csvVar.get()))
        removeEmailsButton.grid(row=4,column=2, sticky=tk.W+tk.E)
